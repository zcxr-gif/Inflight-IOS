...
- **Everything Apple knows about a field**, from WeatherKit, in one request per airport: the next hour minute by minute, twenty-four hours of forecast with wind and gusts, ten days of outlook, sunrise, sunset, civil twilight and the moon, and the severe-weather alerts with the agency that issued each one. Conditions and wind for the large majority of airfields that issue no METAR at all — and where one *is* filed, the report always wins. Best of the lot: **wind on the runways**, worked from the filed wind against each runway centreline as mapped, so the field's panel says which end the wind favours and how much of it is across. Apple's attribution requirements, and where each mark is drawn, are in [`ios-native/WEATHERKIT.md`](ios-native/WEATHERKIT.md).
