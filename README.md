...
- **Forecast, alerts and the fields that file nothing**, from Apple's WeatherKit — an hourly strip and severe-weather alerts on every airport panel, and conditions for the large majority of airfields that issue no METAR at all. A filed METAR always wins where there is one. Apple's attribution requirements, and where each mark is drawn, are in [`ios-native/WEATHERKIT.md`](ios-native/WEATHERKIT.md).
