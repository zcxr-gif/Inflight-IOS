...
- **Real-world traffic**, off until you ask for it. Settings › Real-world traffic draws actual aeroplanes alongside the simulator's, from ADS-B — the position reports real aircraft broadcast and a network of volunteer receivers publishes. They are painted mint so you can always tell them from the server's traffic, and tapping one gives its callsign, type, height and speed and nothing else: there is no pilot behind an ADS-B contact, no filed plan on our backend and no history to replay, so no flight window opens. **It is loud about being on** — a bar stands over the map for as long as the layer does, saying how many real aircraft are in range, with an OFF button on it — because every other count in the app is about Infinite Flight and a map quietly carrying two hundred real aeroplanes makes all of them look wrong. Not a certified source and never presented as one: coverage is wherever volunteers have receivers, and nothing about it is fit for any operational purpose. The whole design, and what it deliberately does not touch, is in [`ios-native/REALWORLD.md`](ios-native/REALWORLD.md).
...
- **A forecast for every field**, in one request per airport: the next two hours a quarter of an hour at a time, twenty-four hours of forecast with wind and gusts, ten days of outlook, and — worked out on the device rather than fetched, so they are there with no signal — sunrise, sunset, civil twilight and the moon. Severe-weather warnings from the National Weather Service where it issues them. Conditions and wind for the large majority of airfields that file no METAR at all, and where one *is* filed the report always wins. Best of the lot: **wind on the runways**, worked from the filed wind against each runway centreline as mapped, so the field's panel says which end the wind favours and how much of it is across. This used to be Apple's WeatherKit and never once worked in a shipped build — the framework refuses locally without the capability on the App ID, so the calls never left the device and the dashboard read zero. It is Open-Meteo now, which the app was already using for the winds aloft: no key, no entitlement, nothing to fail silently. Every source, and what it answers, is in [`ios-native/WEATHER.md`](ios-native/WEATHER.md).
- **Controlled airspace**, under Filters › Controlled airspace, and **Pro**. Every centre with somebody actually working it gets its airspace outlined and its station named — who is on, written on the sky they are controlling. Only staffed sectors: the whole boundary network is nine hundred polygons and would be a second map drawn over the first. **Off until you turn it on.** The boundaries are the VATSIM FIR/UIR set the web tracker shipped, and the match from a station name to a sector is resolved at build time out of `VATSpy.dat` — so a centre named after a FIR (`BIRD`) and one named after an airport inside it (`KLAX` → Los Angeles Centre) both light the right airspace, which is the part the web build never managed. Drawn on the flat map as a wash and an edge, and on the planet as the edge alone — a sector crossing the horizon is a ring cut in half, and there is no honest closed shape to fill. Who is on frequency stays free, in the ATC panel and on every field.
...
There is a second, optional source — real-world ADS-B, swept around wherever the
map is pointed while **Settings › Real-world traffic** is on. It is a layer
rather than a feed: it never reaches the logbook, the widgets, a Live Activity,
the watchlist or a flight window, and it is off on every install. See
[`ios-native/REALWORLD.md`](ios-native/REALWORLD.md).
