...
init(supabaseClient) {
    ...
    open(user) {
        ...
        requestAnimationFrame(() => requestAnimationFrame(() => {
            ...
            // Tab bar is laid out now — settle the glass indicator into place.
            this._positionTabIndicator();
        }));

        // Keep the sliding indicator aligned through rotation / viewport shifts.
        if (!this._tabIndicatorBound) {
            this._tabIndicatorBound = true;
            const reflow = () => { if (this._isOpen) this._positionTabIndicator(); };
    ...
    _injectShell() {
        ...
        el.innerHTML = `
            ...
            <nav id="mdui-tabbar" role="tablist">
                <div id="mdui-tab-indicator" aria-hidden="true"></div>
                ${this._tabDef().map(t => {
    ...
    _render() {
        ...
        // present their navigation. The content area replays a spring page
        // transition on every tab change for a native push feel.
        screen.innerHTML = `
            ${this._renderNavBar()}
            <div class="mdui-content mdui-scroll mdui-page-enter" id="mdui-content-area">
    ...
    /**
     * Slides the liquid-glass capsule behind the active tab button. The tab
     * bar persists across content re-renders, so positioning it here keeps
     * the indicator in sync with whatever tab just became active.
     */
    _positionTabIndicator() {
        const bar = document.getElementById('mdui-tabbar');
        const ind = document.getElementById('mdui-tab-indicator');
        if (!bar || !ind) return;
        // Hidden entirely during onboarding (no tab bar shown).
        if (this._activeTab === 'onboarding') { ind.classList.remove('ready'); return; }
        const btn = bar.querySelector(`.mdui-tab-btn[data-tab="${this._activeTab}"]`);
        if (!btn) { ind.classList.remove('ready'); return; }
        const apply = () => {
            const barRect = bar.getBoundingClientRect();
            const r = btn.getBoundingClientRect();
            if (!r.width) return;
            const left = r.left - barRect.left;
            const firstShow = !ind.classList.contains('ready');
            // Snap (no slide) on first reveal; spring-slide on later switches.
            if (firstShow) ind.style.transition = 'none';
            ind.style.width = `${r.width}px`;
            ind.style.transform = `translateX(${left}px)`;
            if (firstShow) {
                // Force the snapped geometry to commit, then restore transitions.
                void ind.offsetWidth;
                ind.style.transition = '';
                ind.classList.add('ready');
            }
        };
        // Wait a frame so layout is settled after the innerHTML swap.
...
document.getElementById('mdui-billing-cancel')?.addEventListener('click', () => {
    ...
    _injectStyles() {
        ...
        const css = `
            /* ════════════════════════════════════════════════════════════════
               MobileDashboardUI — "Liquid Glass" native iOS design system
               ────────────────────────────────────────────────────────────────
               • Depth-mesh canvas behind everything (no flat gray plate)
               • True translucent glass for floating chrome: nav bar, tab bar,
                 toasts, sheets and modals (saturated backdrop blur)
               • Content cards are layered opaque materials with a hairline
                 glass edge + soft elevation for crisp, readable text
               • Concentric corner radii, SF Pro type scale, system colours
               • Spring motion everywhere; a sliding glass tab indicator
               All --mdui-* token NAMES are preserved (HTML inline styles depend
               on them); values + structure are rebuilt.
               ════════════════════════════════════════════════════════════════ */
            ...
            .mdui-3d-close-btn {
                ...
                color: #fff; border: 0.5px solid rgba(255,255,255,0.22);
            ...
            #mdui-shell, .mdui-wrapper-layer {
                ...
                --mdui-bg:            #eef0f5;   /* base under the depth mesh */
                --mdui-surface:       rgba(120,120,128,0.10); /* recessed inner fill */
                --mdui-card:          #ffffff;   /* primary content material */
                --mdui-card-elev:     #ffffff;   /* lifted (segmented selection) */
                --mdui-card-translucent: rgba(255,255,255,0.78);
                --mdui-input:         rgba(118,118,128,0.12); /* tertiary fill */
                --mdui-glass:         rgba(248,248,252,0.62); /* floating glass */
                --mdui-glass-strong:  rgba(250,250,254,0.80);

                /* Hairlines & separators */
                --mdui-border:        rgba(60,60,67,0.14);
                --mdui-border-light:  rgba(60,60,67,0.08);
                --mdui-border-strong: rgba(60,60,67,0.26);
                --mdui-hover:         rgba(60,60,67,0.06);

                /* Edge highlight that gives materials their "glass" rim */
                --mdui-edge:          rgba(255,255,255,0.65);

                /* Text */
                --mdui-text:          #1c1c1e;
                --mdui-muted:         rgba(60,60,67,0.62);
                --mdui-tertiary:      rgba(60,60,67,0.34);
                ...
                --mdui-accent-glow:   rgba(0,122,255,0.28);
                ...
                --mdui-blur:      saturate(190%) blur(30px);
                --mdui-blur-light: saturate(180%) blur(18px);
                --mdui-shadow-card: 0 1px 3px rgba(0,0,0,0.05), 0 8px 24px -16px rgba(0,0,0,0.18);
                --mdui-shadow-pop:  0 16px 50px rgba(0,0,0,0.20), 0 2px 8px rgba(0,0,0,0.10);
                --mdui-shadow-float: 0 12px 38px rgba(0,0,0,0.16), 0 1px 0 var(--mdui-edge) inset;
                --mdui-spring: cubic-bezier(0.32, 0.72, 0, 1);
                --mdui-spring-soft: cubic-bezier(0.16, 1, 0.3, 1);
            }
            ...
            #mdui-shell {
                ...
                transition: transform 0.52s var(--mdui-spring);
                will-change: transform;
            }
            #mdui-shell.mdui-open { transform: translateY(0); }

            /* Depth mesh: soft accent-tinted glows that give the canvas life
               without per-card blur (keeps scrolling smooth). */
            #mdui-shell::before {
                content: ""; position: absolute; inset: 0; z-index: 0;
                pointer-events: none;
                background:
                    radial-gradient(120% 60% at 12% -8%, var(--mdui-accent-soft), transparent 60%),
                    radial-gradient(110% 55% at 100% 0%, rgba(94,92,230,0.10), transparent 55%),
                    radial-gradient(140% 70% at 50% 112%, rgba(90,200,250,0.10), transparent 60%);
                opacity: 0.9;
            }
            #mdui-shell > * { position: relative; z-index: 1; }

            /* ══════════════════════════ TOKENS — DARK ═══════════════════════ */
            #mdui-shell[data-theme="dark"], .mdui-wrapper-layer[data-theme="dark"] {
                --mdui-bg:            #050507;
                --mdui-surface:       rgba(120,120,128,0.18);
                --mdui-card:          #161618;
                --mdui-card-elev:     #2c2c2e;
                --mdui-card-translucent: rgba(28,28,32,0.74);
                --mdui-input:         rgba(120,120,128,0.26);
                --mdui-glass:         rgba(28,28,32,0.58);
                --mdui-glass-strong:  rgba(36,36,42,0.78);

                --mdui-border:        rgba(120,120,128,0.36);
                --mdui-border-light:  rgba(120,120,128,0.20);
                --mdui-border-strong: rgba(120,120,128,0.55);
                --mdui-hover:         rgba(235,235,245,0.10);

                --mdui-edge:          rgba(255,255,255,0.10);

                --mdui-text:          #ffffff;
                --mdui-muted:         rgba(235,235,245,0.62);
                --mdui-tertiary:      rgba(235,235,245,0.32);
                --mdui-on-accent:     #ffffff;

                --mdui-accent:        #0a84ff;
                --mdui-accent-hover:  #409cff;
                --mdui-accent-soft:   rgba(10,132,255,0.24);
                --mdui-accent-glow:   rgba(10,132,255,0.40);

                --mdui-success:       #30d158; --mdui-success-soft: rgba(48,209,88,0.22);
                --mdui-danger:        #ff453a; --mdui-danger-soft:  rgba(255,69,58,0.20);
                --mdui-warn:          #ff9f0a; --mdui-warn-soft:    rgba(255,159,10,0.22);
                --mdui-info:          #64d2ff; --mdui-info-soft:    rgba(100,210,255,0.22);

                --mdui-shadow-card: 0 1px 3px rgba(0,0,0,0.6), 0 10px 28px -18px rgba(0,0,0,0.8);
                --mdui-shadow-pop:  0 18px 56px rgba(0,0,0,0.7), 0 2px 10px rgba(0,0,0,0.5);
                --mdui-shadow-float: 0 14px 42px rgba(0,0,0,0.6), 0 0.5px 0 var(--mdui-edge) inset;
            }
            #mdui-shell[data-theme="dark"]::before {
                background:
                    radial-gradient(120% 60% at 10% -10%, var(--mdui-accent-glow), transparent 58%),
                    radial-gradient(120% 60% at 100% 0%, rgba(94,92,230,0.20), transparent 55%),
                    radial-gradient(150% 75% at 50% 115%, rgba(48,209,88,0.10), transparent 60%);
                opacity: 0.7;
            }
            ...
            .mdui-wrapper-layer.mdui-open .mdui-confirm-box,
            .mdui-wrapper-layer.mdui-open .mdui-drill-box {
                animation: mdui-modal-pop 0.4s var(--mdui-spring-soft) both;
            }
            @keyframes mdui-modal-pop { from { opacity: 0; transform: translateY(16px) scale(0.94); } to { opacity: 1; transform: translateY(0) scale(1); } }
            ...
            #mdui-sheet-grabber span {
                width: 38px; height: 5px; margin-top: 7px;
                border-radius: 999px;
            ...
            .mdui-nav-title {
                ...
                transition: opacity 0.28s ease, transform 0.28s var(--mdui-spring-soft);
            }
            ...
            .mdui-nav-avatar {
                ...
                background: linear-gradient(155deg, var(--mdui-accent-hover), var(--mdui-accent));
                color: var(--mdui-on-accent);
                border-radius: 50%;
                display: flex; align-items: center; justify-content: center;
                font-family: inherit; font-size: 12px; font-weight: 700;
                flex-shrink: 0; user-select: none;
                box-shadow: 0 3px 10px var(--mdui-accent-glow), inset 0 0.5px 0 rgba(255,255,255,0.4);
            }
            ...
            .mdui-large-title {
                ...
                letter-spacing: -1.1px; line-height: 1.04;
                color: var(--mdui-text);
            }
            .mdui-large-subtitle {
                margin: 6px 0 0;
            ...
            /* Hide the legacy top-bar inside the live screen only. */
            ...
            .mdui-avatar {
                ...
                box-shadow: 0 3px 10px var(--mdui-accent-glow);
            }

            /* ══════════════════════════ CONTENT SCROLLER ═══════════════════ */
            .mdui-content {
                flex: 1; overflow-y: auto; min-height: 0;
                padding: 0 var(--mdui-page-pad) calc(var(--mdui-tab-h) + env(safe-area-inset-bottom) + 46px);
            ...
            /* Page transition: each tab's content slides+fades up on switch. */
            @keyframes mdui-page-enter { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
            .mdui-page-enter { animation: mdui-page-enter 0.4s var(--mdui-spring-soft) both; }

            /* ════════════════════ iOS INSET GROUPED LISTS ══════════════════ */
            .mdui-list-section { margin-bottom: 28px; }
            ...
            .mdui-list-section-footer {
                ...
                color: var(--mdui-muted);
                padding: 8px 18px 0; line-height: 1.4;
            }
            .mdui-list-group {
            ...
            .mdui-list-group::before {
                content: ""; position: absolute; inset: 0; pointer-events: none;
                border-radius: inherit;
                box-shadow: inset 0 0.5px 0 var(--mdui-edge);
            }
            .mdui-list-row {
                position: relative;
            ...
            .mdui-list-row:not(:last-child)::after {
                ...
                left: 58px; right: 0; bottom: 0; height: 0.5px;
            ...
            .mdui-list-row-icon {
                ...
                box-shadow: inset 0 0.5px 0 rgba(255,255,255,0.35), 0 2px 5px rgba(0,0,0,0.10);
            }
            .mdui-list-row-icon.tone-gray   { background: linear-gradient(160deg,#a0a0a6,#8e8e93); }
            .mdui-list-row-icon.tone-green  { background: linear-gradient(160deg,#34d364,var(--mdui-success)); }
            .mdui-list-row-icon.tone-red    { background: linear-gradient(160deg,#ff5b52,var(--mdui-danger)); }
            .mdui-list-row-icon.tone-orange { background: linear-gradient(160deg,#ffab33,var(--mdui-warn)); }
            .mdui-list-row-icon.tone-purple { background: linear-gradient(160deg,#bf63e6,#af52de); }
            .mdui-list-row-icon.tone-blue   { background: linear-gradient(160deg,#3a9bff,#0a84ff); }
            .mdui-list-row-icon.tone-indigo { background: linear-gradient(160deg,#7a78ef,#5e5ce6); }
            .mdui-list-row-text {
            ...
            /* iOS Settings identity hero */
            .mdui-id-card {
                position: relative;
                display: flex; align-items: center; gap: 15px;
                width: 100%; padding: 16px; margin-bottom: 28px;
                border: 0.5px solid var(--mdui-border-light);
                background: var(--mdui-card);
                ...
                box-shadow: var(--mdui-shadow-card);
                -webkit-tap-highlight-color: transparent;
                transition: background-color 0.14s ease, transform 0.18s var(--mdui-spring-soft);
            }
            .mdui-id-card::before {
                content: ""; position: absolute; inset: 0; pointer-events: none;
                border-radius: inherit; box-shadow: inset 0 0.5px 0 var(--mdui-edge);
            }
            .mdui-id-card:active { background: var(--mdui-hover); transform: scale(0.985); }
            .mdui-id-avatar {
                flex: 0 0 auto; width: 60px; height: 60px; border-radius: 50%;
                background: linear-gradient(160deg, var(--mdui-accent-hover), var(--mdui-accent));
                color: #fff; display: grid; place-items: center;
                font-size: 23px; font-weight: 700;
                box-shadow: 0 6px 16px var(--mdui-accent-glow), inset 0 0.5px 0 rgba(255,255,255,0.4);
            }
            ...
            .mdui-segmented-opt {
                ...
                color: var(--mdui-text); cursor: pointer;
                -webkit-tap-highlight-color: transparent;
                transition: color 0.18s ease;
            }
            .mdui-segmented-opt input { display: none; }
            .mdui-segmented-opt:has(input:checked) {
                background: var(--mdui-card-elev); font-weight: 600;
                box-shadow: 0 1px 5px rgba(0,0,0,0.16), 0 0 0 0.5px rgba(0,0,0,0.04);
            }

            /* iOS inline form rows */
            .mdui-list-form-row {
                display: flex; align-items: center; gap: 10px;
                padding: 10px 16px; min-height: 50px; position: relative;
            }
            .mdui-list-form-row:not(:last-child)::after {
                content: ""; position: absolute;
                left: 16px; right: 0; bottom: 0; height: 0.5px;
                background: var(--mdui-border-light);
            }
            ...
            .mdui-list-form-control select {
                ...
                font-family: inherit; font-size: 17px; letter-spacing: -0.4px;
                outline: none;
            }
            .mdui-list-form-control input::placeholder { color: var(--mdui-tertiary); }
            .mdui-list-form-control input:disabled {
            ...
            #mdui-tabbar {
                ...
                border-radius: var(--mdui-radius-2xl);
                display: flex; z-index: 9999; padding: 6px;
                box-shadow: var(--mdui-shadow-float);
                transition: transform 0.36s var(--mdui-spring-soft), opacity 0.2s ease;
            }
            /* Sliding liquid-glass indicator behind the active tab */
            #mdui-tab-indicator {
                position: absolute;
                top: 6px; left: 0;
                height: calc(var(--mdui-tab-h) - 12px);
                width: 0; /* set by JS */
                border-radius: calc(var(--mdui-radius-2xl) - 8px);
                background: var(--mdui-accent-soft);
                box-shadow: inset 0 0.5px 0 var(--mdui-edge);
                z-index: 0; opacity: 0;
                transform: translateX(0);
                transition: transform 0.46s var(--mdui-spring), width 0.46s var(--mdui-spring), opacity 0.2s ease;
                pointer-events: none;
            }
            #mdui-tab-indicator.ready { opacity: 1; }
            .mdui-tab-btn {
                ...
                -webkit-tap-highlight-color: transparent;
                transition: color 0.24s ease, transform 0.2s var(--mdui-spring-soft);
            }
            .mdui-tab-iconwrap { position: relative; display: grid; place-items: center; line-height: 0; }
            .mdui-tab-btn i { font-size: 20px; line-height: 1; transition: transform 0.26s var(--mdui-spring-soft); }
            ...
            .mdui-card-header { padding: 15px 18px 10px; position: relative; }
            .mdui-card-header h3 {
                margin: 0; font-size: 17px; font-weight: 700;
                color: var(--mdui-text); letter-spacing: -0.4px;
                display: flex; align-items: center;
            }
            .mdui-card-body { padding: 18px; position: relative; }
            .mdui-card-eyebrow {
            ...
            /* ══════════════════════════ INPUTS ═════════════════════════════ */
            .mdui-input-group { margin-bottom: 15px; }
            .mdui-input-group label {
                display: block; font-size: 13px; font-weight: 600;
                color: var(--mdui-muted); margin-bottom: 8px; letter-spacing: -0.1px;
            }
            .mdui-input-wrap { position: relative; display: flex; align-items: center; }
            .mdui-input-icon { position: absolute; left: 14px; color: var(--mdui-tertiary); font-size: 14px; pointer-events: none; }
            .mdui-input {
            ...
            .mdui-input.has-icon { padding-left: 40px; }
            .mdui-input:focus {
                background: var(--mdui-card-elev);
                border-color: var(--mdui-accent);
                box-shadow: 0 0 0 4px var(--mdui-accent-soft);
            }
            ...
            .mdui-help { font-size: 13px; color: var(--mdui-tertiary); margin-top: 8px; line-height: 1.4; letter-spacing: -0.1px; }
            ...
            .mdui-btn-primary, .mdui-btn-secondary, .mdui-btn-danger-outline, .mdui-btn-ghost, .mdui-btn-danger {
                display: inline-flex; align-items: center; justify-content: center;
                gap: 8px; padding: 13px 18px; border-radius: 14px;
                font-family: var(--mdui-font-sans);
            ...
            .mdui-btn-primary {
                background: linear-gradient(165deg, var(--mdui-accent-hover), var(--mdui-accent));
                color: var(--mdui-on-accent); border: none;
                box-shadow: 0 6px 18px var(--mdui-accent-glow), inset 0 0.5px 0 rgba(255,255,255,0.35);
            }
            .mdui-btn-primary:hover { filter: brightness(1.04); }
            .mdui-btn-primary:active { transform: scale(0.97); opacity: 0.94; box-shadow: 0 3px 10px var(--mdui-accent-glow); }
            .mdui-btn-primary:disabled { opacity: 0.5; cursor: not-allowed; box-shadow: none; }
            .mdui-btn-block { width: 100%; padding: 15px 20px; }
            .mdui-btn-secondary { background: var(--mdui-input); color: var(--mdui-accent); border: none; width: 100%; }
            ...
            .mdui-btn-danger { background: var(--mdui-danger); color: #fff; border: none; box-shadow: 0 6px 18px var(--mdui-danger-soft); }
            ...
            .mdui-fade-up { animation: mdui-fade-up 0.42s var(--mdui-spring-soft) both; }
            @keyframes mdui-pulse { 0%, 100% { opacity: 0.5; } 50% { opacity: 0.9; } }
            .mdui-skel {
                background: linear-gradient(100deg, var(--mdui-input) 30%, var(--mdui-hover) 50%, var(--mdui-input) 70%);
            ...
            .mdui-empty-state i { font-size: 36px; color: var(--mdui-tertiary); opacity: 0.7; margin-bottom: 15px; display: block; }
            .mdui-empty-state h4 { margin: 0 0 6px 0; color: var(--mdui-text); font-size: 17px; font-weight: 700; letter-spacing: -0.4px; }
            .mdui-empty-state p { margin: 0; color: var(--mdui-muted); font-size: 14px; line-height: 1.45; letter-spacing: -0.2px; }
            .mdui-empty {
                display: flex; flex-direction: column; align-items: center; justify-content: center;
                gap: 8px; padding: 30px 14px; color: var(--mdui-muted);
                font-size: 14px; font-weight: 500; text-align: center; letter-spacing: -0.2px;
            }
            .mdui-empty i { font-size: 26px; opacity: 0.6; }

            /* ══════════════════════════ TOASTS ═════════════════════════════ */
            .mdui-toast-container {
                position: fixed;
                bottom: calc(var(--mdui-tab-h) + env(safe-area-inset-bottom) + 20px);
            ...
            .mdui-toast {
                ...
                pointer-events: auto; animation: mdui-toast-in 280ms var(--mdui-spring-soft);
            ...
            /* ══════════════════════════ DASHBOARD ══════════════════════════ */
            .mdui-greeting-hero { padding: 2px 2px 22px; }
            ...
            .mdui-greeting-name {
                ...
                color: var(--mdui-text); line-height: 1.1; letter-spacing: -0.8px;
            }
            .mdui-stat-strip {
                display: flex; align-items: center; gap: 7px; flex-wrap: wrap;
                margin-top: 11px; color: var(--mdui-muted);
            ...
            .mdui-dispatch-icao {
                font-family: var(--mdui-font-mono); font-size: 25px; font-weight: 700;
            ...
            .mdui-dispatch-center {
                ...
                color: var(--mdui-tertiary); flex: 1;
                font-size: 10.5px; font-weight: 600; letter-spacing: 0.02em;
            }
            .mdui-dispatch-plane { color: var(--mdui-accent); font-size: 14px; }
            .mdui-dispatch-dur { font-size: 11px; font-weight: 600; color: var(--mdui-muted); }
            ...
            .mdui-flight-row { display: flex; align-items: center; gap: 12px; padding: 13px 16px; border-bottom: 0.5px solid var(--mdui-border-light); }
            ...
            .mdui-live-stack { display: flex; flex-direction: column; gap: 13px; margin-bottom: 24px; }
            .mdui-live-card {
                background: var(--mdui-card);
                border: 0.5px solid var(--mdui-border-light);
                border-radius: var(--mdui-radius-xl);
            ...
            .mdui-live-visual { position: relative; height: 196px; background: #0a0e14; overflow: hidden; }
            ...
            .mdui-live-bg-blur { position: absolute; inset: -24px; background-size: cover; background-position: center; filter: blur(24px) brightness(0.55); z-index: 1; }
            .mdui-live-bg-fallback { background: linear-gradient(135deg, #2a3445 0%, #1a2030 60%, #14181f 100%); z-index: 1; }
            .mdui-live-overlay {
                position: absolute; inset: 0; z-index: 3; pointer-events: none;
                background:
                    linear-gradient(to top, rgba(0,0,0,0.68) 0%, rgba(0,0,0,0.12) 38%, transparent 62%),
            ...
            .mdui-live-image-cs { font-family: var(--mdui-font-mono); font-size: 19px; font-weight: 700; letter-spacing: 0.02em; line-height: 1.1; }
            ...
            .mdui-live-content { display: flex; flex-direction: column; gap: 15px; padding: 17px; }
            .mdui-live-route { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
            .mdui-live-icao { font-family: var(--mdui-font-mono); font-size: 25px; font-weight: 700; letter-spacing: -0.5px; color: var(--mdui-text); line-height: 1; }
            ...
            .mdui-live-stats { display: grid; grid-template-columns: repeat(3, 1fr); background: var(--mdui-input); border-radius: 14px; overflow: hidden; }
            .mdui-live-stat { display: flex; flex-direction: column; gap: 3px; padding: 12px 8px; border-right: 0.5px solid var(--mdui-border-light); align-items: center; min-width: 0; }
            .mdui-live-stat:last-child { border-right: none; }
            .mdui-live-stat .label { font-size: 10.5px; font-weight: 500; letter-spacing: -0.1px; color: var(--mdui-muted); }
            ...
            .mdui-live-action {
                ...
                width: 100%; padding: 15px 16px; border: none; border-radius: 14px;
                background: linear-gradient(165deg, var(--mdui-accent-hover), var(--mdui-accent)); color: var(--mdui-on-accent);
            ...
            .mdui-live-action:active { transform: scale(0.985); opacity: 0.92; box-shadow: 0 3px 10px var(--mdui-accent-glow); }
            ...
            .mdui-dossier-hero {
                ...
                margin-bottom: 24px; box-shadow: var(--mdui-shadow-card);
            }
            ...
            .mdui-dossier-avatar {
                ...
                box-shadow: 0 6px 16px rgba(0,0,0,0.28), inset 0 0.5px 0 rgba(255,255,255,0.35);
            }
            .mdui-dossier-id { flex: 1 1 auto; min-width: 0; display: flex; flex-direction: column; gap: 3px; }
            .mdui-dossier-name { font-size: 23px; font-weight: 700; letter-spacing: -0.5px; color: var(--mdui-text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
            .mdui-dossier-handle {
                display: inline-flex; align-items: center; gap: 6px;
            ...
            .mdui-dossier-grade strong { font-size: 23px; font-weight: 700; line-height: 1; color: var(--mdui-accent); }
            .mdui-dossier-hero.has-cover .mdui-dossier-grade strong { color: #fff; }

            .mdui-cover-banner { width: 100%; height: 144px; margin-bottom: 18px; border-radius: var(--mdui-radius-lg); background-size: cover; background-position: center; position: relative; overflow: hidden; border: 0.5px solid var(--mdui-border-light); }
            ...
            .mdui-cover-name { font-size: 23px; font-weight: 700; letter-spacing: -0.4px; }
            .mdui-cover-bio { margin-top: 3px; font-size: 14px; opacity: 0.92; letter-spacing: -0.2px; }
            .mdui-bio-strip { display: flex; align-items: center; gap: 10px; padding: 13px 15px; margin-bottom: 18px; background: var(--mdui-accent-soft); border: none; border-radius: var(--mdui-radius); color: var(--mdui-text); font-size: 14px; line-height: 1.4; font-style: italic; letter-spacing: -0.2px; }

            /* Dossier sub-tab segmented control */
            .mdui-sub-tab-bar {
                display: flex; gap: 2px; padding: 3px;
                background: var(--mdui-input); border: none;
            ...
            .mdui-career-tab-btn {
                ...
                -webkit-tap-highlight-color: transparent;
                transition: background-color 0.2s ease, color 0.2s ease, transform 0.18s var(--mdui-spring-soft);
            }
            .mdui-career-tab-btn i { font-size: 12px; }
            .mdui-career-tab-btn:active { transform: scale(0.97); }
            .mdui-career-tab-btn.active {
                background: var(--mdui-card-elev); color: var(--mdui-text); font-weight: 600;
                box-shadow: 0 1px 5px rgba(0,0,0,0.14), 0 0 0 0.5px rgba(0,0,0,0.04);
            }
            #mdui-career-content-host { min-height: 200px; }

            .mdui-mini-stat-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 11px; }
            .mdui-mini-stat-card {
                position: relative;
                background: var(--mdui-card);
                border: 0.5px solid var(--mdui-border-light);
                ...
                box-shadow: var(--mdui-shadow-card); min-width: 0;
            }
            .mdui-mini-stat-card::before { content: ""; position: absolute; inset: 0; border-radius: inherit; pointer-events: none; box-shadow: inset 0 0.5px 0 var(--mdui-edge); }
            .mdui-drillable-card { cursor: pointer; transition: transform 0.18s var(--mdui-spring-soft), background-color 0.16s ease; -webkit-tap-highlight-color: transparent; }
            ...
            .mdui-mini-value { font-size: 18px; font-weight: 700; color: var(--mdui-text); line-height: 1; letter-spacing: -0.4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
            .mdui-mini-label { font-size: 10.5px; font-weight: 600; color: var(--mdui-muted); text-transform: uppercase; letter-spacing: 0.04em; }

            .mdui-trends-grid { display: grid; gap: 2px; }
            .mdui-trend-row { display: grid; grid-template-columns: 1.4fr minmax(0, 1fr) minmax(0, 1fr); align-items: center; gap: 8px; padding: 12px 0; border-bottom: 0.5px solid var(--mdui-border-light); }
            .mdui-trend-row:last-child { border-bottom: none; }
            .mdui-trend-label { font-size: 15px; color: var(--mdui-muted); letter-spacing: -0.2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
            .mdui-trend-current { font-size: 16px; font-weight: 600; color: var(--mdui-text); letter-spacing: -0.3px; overflow: hidden; text-overflow: ellipsis; }
            .mdui-trend-delta { font-size: 14px; font-weight: 600; overflow: hidden; text-overflow: ellipsis; display: flex; align-items: center; gap: 3px; }
            ...
            .mdui-records-grid { display: grid; gap: 11px; }
            .mdui-record { padding: 15px 16px; border-radius: var(--mdui-radius); border: 0.5px solid var(--mdui-border-light); background: var(--mdui-card); box-shadow: var(--mdui-shadow-card); }
            .mdui-record[data-tone="indigo"]  { background: rgba(94,92,230,0.07);  border-color: rgba(94,92,230,0.18); }
            .mdui-record[data-tone="emerald"] { background: rgba(48,209,88,0.07);   border-color: rgba(48,209,88,0.18); }
            .mdui-record[data-tone="amber"]   { background: rgba(255,149,0,0.07);   border-color: rgba(255,149,0,0.20); }
            ...
            .mdui-record[data-tone="emerald"] .mdui-record-icon { color: #30d158; }
            .mdui-record[data-tone="amber"] .mdui-record-icon { color: #ff9500; }
            ...
            .mdui-node-header { display: flex; justify-content: space-between; align-items: center; padding: 17px 18px; border-bottom: 0.5px solid var(--mdui-border-light); }
            .mdui-node-icao { font-family: var(--mdui-font-mono); font-size: 21px; font-weight: 700; color: var(--mdui-text); letter-spacing: -0.3px; }
            .mdui-assessment-row { display: flex; align-items: flex-start; gap: 12px; padding: 13px 14px; background: var(--mdui-surface); border-radius: 14px; margin: 16px 18px; }
            ...
            .mdui-load-bar-track { width: 100%; height: 8px; background: var(--mdui-input); border-radius: 5px; overflow: hidden; }
            .mdui-load-bar-fill { height: 100%; border-radius: 5px; transition: width 0.5s var(--mdui-spring-soft); }
            .mdui-intel-stats-quad { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 10px; padding: 15px 18px; border-top: 0.5px solid var(--mdui-border-light); border-bottom: 0.5px solid var(--mdui-border-light); }
            .mdui-intel-stat { display: flex; flex-direction: column; gap: 4px; min-width: 0; }
            .mdui-intel-stat .label { font-size: 10px; color: var(--mdui-tertiary); font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; }
            .mdui-intel-stat .value { font-family: var(--mdui-font-mono); font-size: 21px; color: var(--mdui-text); font-weight: 700; line-height: 1; overflow: hidden; text-overflow: ellipsis; }
            .mdui-section-head { display: flex; justify-content: space-between; align-items: center; margin: 16px 18px 8px; }
            .mdui-intel-heatmap { display: flex; gap: 3px; height: 32px; border-radius: 10px; overflow: hidden; background: var(--mdui-input); padding: 3px; margin: 0 18px 16px; }
            .mdui-intel-heatblock { flex: 1; border-radius: 6px; }

            /* ══════════════════════════ DISPATCH ═══════════════════════════ */
            .mdui-form-sheet { margin-bottom: 18px; }
            .mdui-form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
            .mdui-form-section-title { margin: 4px 0 12px; font-size: 11px; font-weight: 700; color: var(--mdui-tertiary); text-transform: uppercase; letter-spacing: 0.06em; padding-bottom: 8px; border-bottom: 0.5px solid var(--mdui-border-light); }
            .mdui-form-actions { margin-top: 18px; padding-top: 14px; border-top: 0.5px solid var(--mdui-border-light); display: flex; gap: 10px; justify-content: flex-end; }
            ...
            .mdui-ticket-stub { border-bottom: 0.5px dashed var(--mdui-border); padding: 15px 16px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
            ...
            .mdui-ticket-cs { background: var(--mdui-accent-soft); color: var(--mdui-accent); padding: 5px 10px; border-radius: 8px; font-size: 12px; font-weight: 700; font-family: var(--mdui-font-mono); }
            .mdui-ticket-body { padding: 17px; }
            ...
            .mdui-ticket-icao { font-family: var(--mdui-font-mono); font-size: 25px; font-weight: 700; color: var(--mdui-text); letter-spacing: -0.5px; line-height: 1; }
            ...
            .mdui-ticket-aside { border-top: 0.5px solid var(--mdui-border-light); padding: 13px 16px; display: flex; justify-content: space-around; flex-wrap: wrap; gap: 6px; }
            ...
            .mdui-confirm-icon { font-size: 30px; color: var(--mdui-danger); margin-bottom: 13px; width: 58px; height: 58px; border-radius: 50%; background: var(--mdui-danger-soft); display: grid; place-items: center; margin-left: auto; margin-right: auto; }
            ...
            .mdui-confirm-flight-plate { background: var(--mdui-surface); padding: 13px; border-radius: 12px; margin: 16px 0; display: flex; align-items: center; justify-content: center; gap: 10px; }
            ...
            .mdui-autocomplete-item { padding: 12px 14px; display: flex; align-items: center; gap: 10px; border-bottom: 0.5px solid var(--mdui-border-light); cursor: pointer; font-weight: 500; font-size: 15px; color: var(--mdui-text); }
            ...
            .mdui-watchlist-grid { display: grid; gap: 11px; }
            .mdui-wl-card { position: relative; background: var(--mdui-card); border: 0.5px solid var(--mdui-border-light); border-radius: var(--mdui-radius); padding: 15px; box-shadow: var(--mdui-shadow-card); }
            .mdui-wl-card.live { border-color: var(--mdui-success); }
            ...
            .mdui-wl-username { font-weight: 600; font-size: 16px; letter-spacing: -0.3px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
            ...
            .mdui-wl-flight-info { margin-top: 13px; padding-top: 13px; border-top: 0.5px solid var(--mdui-border-light); }
            .mdui-wl-route { display: flex; align-items: center; justify-content: center; gap: 12px; margin-bottom: 11px; font-family: var(--mdui-font-mono); font-weight: 700; font-size: 18px; letter-spacing: -0.2px; }
            .mdui-wl-plane-icon { color: var(--mdui-accent); font-size: 14px; }
            .mdui-wl-telemetry { display: grid; grid-template-columns: 1fr 1fr; gap: 9px; }
            .mdui-wl-tele-item { display: flex; flex-direction: column; background: var(--mdui-input); padding: 10px 12px; border-radius: 11px; min-width: 0; }
            .mdui-wl-tele-item .label { font-size: 10px; color: var(--mdui-tertiary); font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; }
            ...
            .mdui-toggle-thumb { position: absolute; top: 2px; left: 2px; width: 27px; height: 27px; background: #fff; border-radius: 50%; box-shadow: 0 3px 8px rgba(0,0,0,0.18), 0 1px 1px rgba(0,0,0,0.16); transition: transform 0.3s var(--mdui-spring); }
            ...
            .mdui-drill-header { display: flex; align-items: center; justify-content: space-between; padding: 17px 20px; border-bottom: 0.5px solid var(--mdui-border-light); }
            ...
            .mdui-drill-body { padding: 17px 20px; }
            .mdui-drill-stat-row { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 0.5px solid var(--mdui-border-light); font-size: 15px; }
            ...
            .mdui-drill-bar-label { display: flex; justify-content: space-between; margin-bottom: 6px; color: var(--mdui-muted); }
            .mdui-drill-bar-track { width: 100%; height: 8px; background: var(--mdui-input); border-radius: 5px; overflow: hidden; }
            .mdui-drill-bar-fill { height: 100%; background: var(--mdui-accent); border-radius: 5px; }
            .mdui-drill-note { margin: 0; padding: 13px; background: var(--mdui-info-soft); border-radius: 12px; font-size: 14px; color: var(--mdui-info); font-weight: 500; }
            .mdui-drill-actions { padding: 15px 20px; border-top: 0.5px solid var(--mdui-border-light); display: flex; justify-content: flex-end; }
            ...
            .mdui-theme-options { display: grid; grid-template-columns: 1fr 1fr; gap: 9px; }
            .mdui-theme-option { display: flex; align-items: center; gap: 10px; padding: 13px; border: 0.5px solid var(--mdui-border); border-radius: 13px; cursor: pointer; background: var(--mdui-input); -webkit-tap-highlight-color: transparent; transition: 0.16s; }
            ...
            .mdui-accent-swatch { width: 33px; height: 33px; border-radius: 50%; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 12px; transition: transform 0.16s var(--mdui-spring-soft), box-shadow 0.16s ease; padding: 0; -webkit-tap-highlight-color: transparent; box-shadow: inset 0 0.5px 1px rgba(255,255,255,0.4); }
            .mdui-accent-swatch:active { transform: scale(0.9); }
            .mdui-accent-swatch.active { box-shadow: 0 0 0 2px var(--mdui-card), 0 0 0 4.5px var(--mdui-accent); }
            .mdui-plan-box { background: var(--mdui-input); border: none; border-radius: var(--mdui-radius); padding: 17px; }
            .mdui-plan-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; gap: 8px; }
            .mdui-plan-header h4 { margin: 0; font-size: 16px; font-weight: 600; color: var(--mdui-text); letter-spacing: -0.3px; }
            .mdui-plan-price { font-size: 25px; font-weight: 700; color: var(--mdui-text); margin: 0 0 4px 0; letter-spacing: -0.5px; }
            ...
            /* ══════════════════════════ ONBOARDING ═════════════════════════ */
            .mdui-onboarding { padding: 38px 18px calc(env(safe-area-inset-bottom) + 38px); display: flex; flex-direction: column; justify-content: flex-start; min-height: 100%; }
            .mdui-onb-icon { width: 64px; height: 64px; margin: 0 auto 20px; border-radius: 19px; background: linear-gradient(160deg, var(--mdui-accent-hover), var(--mdui-accent)); color: #fff; font-size: 26px; display: grid; place-items: center; box-shadow: 0 8px 22px var(--mdui-accent-glow), inset 0 0.5px 0 rgba(255,255,255,0.35); }
            .mdui-onb-title { font-size: 32px; font-weight: 800; text-align: center; margin: 0 0 8px; color: var(--mdui-text); letter-spacing: -0.9px; }
            ...
            .mdui-action-link { display: flex; align-items: center; gap: 12px; padding: 13px 14px; background: var(--mdui-input); border: none; border-radius: 13px; text-decoration: none; color: inherit; -webkit-tap-highlight-color: transparent; transition: background-color 0.18s ease, transform 0.16s ease; }
            .mdui-action-link:active { transform: scale(0.98); background: var(--mdui-hover); }
            .mdui-action-icon { width: 33px; height: 33px; border-radius: 50%; background: var(--mdui-accent-soft); color: var(--mdui-accent); display: grid; place-items: center; font-size: 14px; flex-shrink: 0; }
            .mdui-action-text { flex: 1; font-size: 15px; font-weight: 500; color: var(--mdui-text); letter-spacing: -0.2px; min-width: 0; overflow: hidden; text-overflow: ellipsis; }

            /* ══════════════════════ REDUCED MOTION ═════════════════════════ */
            @media (prefers-reduced-motion: reduce) {
                #mdui-shell, .mdui-fade-up, .mdui-page-enter, #mdui-tab-indicator,
                .mdui-toast, .mdui-confirm-box, .mdui-drill-box { animation: none !important; transition-duration: 0.01ms !important; }
            }
        `;
