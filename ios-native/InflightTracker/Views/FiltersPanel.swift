...
/// ## Only filters
///
/// Everything here takes traffic away, and that is now the whole test for
/// belonging in this panel: a row is here if turning it off means fewer
/// aeroplanes on the map. The eleven rows that used to sit above these — the
/// airports layer, the ground layout, the flown path, the day-and-night wash,
/// the tracks, the route line, the callsigns, the VA logos, the airspace —
/// hide nothing. They are preferences about how the map is drawn, they were
/// unreachable from Settings where somebody would look for them, and the RESET
/// button at the top of this panel pointedly left every one of them alone.
/// They live in `MapLayersSettingsPanel` now.
///
/// What is left is four groups and a switch, all of them counted by
/// `MapFilters.activeCount`, all of them undone by RESET, and all of them
/// honestly described by the number in the header.
struct FiltersPanel: View {
    ...
    var body: some View {
        ...
        MapPanel(
            ...
            Text("Filters only change what is drawn. The feed is unchanged, so nothing has to be re-fetched when you turn one back on. What the map draws over the traffic — airports, tracks, routes, callsigns — is in Settings under Layers.")
